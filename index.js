...
async function httpGetWithRetry(url, retries = 3, delayMs = 1000) {
	for (let attempt = 1; attempt <= retries; attempt++) {
		try {
			return await new Promise((resolve, reject) => {
				https.get(url, (resp) => {
					...
					if (resp.statusCode === 404) {
						...
						return;
					} else if (resp.statusCode === 429 || (resp.statusCode < 200 || resp.statusCode > 299)) {
						let error = resp.statusCode === 429 ? 'Rate limit exceeded' : `HTTP status code ${resp.statusCode}`;
						if (attempt < retries) {
							reject(error);
						} else {
			...
			if (attempt < retries) {
				console.log(`Attempt ${attempt} failed for ${url}. Error: ${error}. Retrying in ${delayMs}ms...`);
...
function updateProgressBar(processedPackages, totalPackages) {
	...
	process.stdout.write(`Processing: [${'#'.repeat(processedPackages)}${'.'.repeat(totalPackages - processedPackages)}]`);
}
...
async function getDependenciesAndCheckCompatibility() {
	...
	const promises = Object.keys(dependencies).map(packageName =>
		checkAngularCompatibility(packageName, dependenciesToCheck, totalPackages, dependencies[packageName])
			.then(() => {
				dependenciesToCheck.processedPackages++;
				if (dependenciesToCheck.processedPackages % 5 === 0 || dependenciesToCheck.processedPackages === totalPackages) {
					updateProgressBar(dependenciesToCheck.processedPackages, totalPackages);
				}
			})
			.catch(error => console.error(colorize(`Could not fetch data for package: ${packageName}`, 'red'), error))
	);
